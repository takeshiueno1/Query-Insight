...
export function EmployeesPage() {
  ...
  const query = useQuery({ queryKey: ['employees', keyword], queryFn: () => api<EmployeePage>(`/api/v1/employees?keyword=${encodeURIComponent(keyword)}&size=20`) })
  return (
    <>
      ...
      <form className="search-card" onSubmit={(event) => { event.preventDefault(); setKeyword(input.trim()) }}>
