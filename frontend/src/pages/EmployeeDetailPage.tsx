...
import { employmentStatusLabels, type EmployeeDetail } from '../types'
...
export function EmployeeDetailPage() {
  ...
  return <><div className="page-heading"><div><span className="eyebrow">社員情報</span><h1>社員詳細</h1><p>{employee.employeeNo}</p></div><Link className="secondary-button" to="/employees">一覧へ戻る</Link></div>
    <section className="profile-hero"><div className="avatar">{employee.lastName[0]}{employee.firstName[0]}</div><div><h2>{employee.lastName} {employee.firstName}</h2><p>{employee.departmentName ?? '所属未設定'} / {employee.positionName ?? '役職未設定'}</p></div><span className="status success">{employmentStatusLabels[employee.employmentStatus] ?? '状態不明'}</span></section>
