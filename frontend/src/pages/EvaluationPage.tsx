...
export function EvaluationPage() {
  ...
  return <><div className="page-heading"><div><span className="eyebrow">SCR-009</span><h1>自己評価</h1><p>{evaluation.periodName} / 状態: {evaluation.status}</p></div><span className="status warning">暫定基準</span></div>
    <div className="evaluation-grid"><section className="card sticky-chart"><h2>評価バランス</h2><RadarChart scores={scores} /><p className="notice">等級境界は未承認のため表示しません。各軸の根拠を重視してください。</p></section>
      <section className="card"><h2>評価入力</h2><div className="axis-list">{evaluation.details.map((detail) => <article className="axis-editor" key={detail.axisCode}><div><h3>{detail.displayName}</h3><p>{detail.description}</p></div><div className="level-buttons" role="group" aria-label={`${detail.displayName}のレベル`}>{[1,2,3,4,5].map((level) => <button type="button" key={level} className={draft[detail.axisCode]?.level === level ? 'selected' : ''} onClick={() => setDraft((current) => ({ ...current, [detail.axisCode]: { level, evidence: current[detail.axisCode]?.evidence ?? '' } }))}>{level}</button>)}</div><label>根拠<textarea maxLength={1500} value={draft[detail.axisCode]?.evidence ?? ''} onChange={(event) => setDraft((current) => ({ ...current, [detail.axisCode]: { level: current[detail.axisCode]?.level ?? 1, evidence: event.target.value } }))} /></label></article>)}</div>
        {message && <div className={mutation.isError ? 'error-banner' : 'success-banner'} role="status">{message}</div>}
        <div className="form-actions"><button className="secondary-button" disabled={mutation.isPending} onClick={() => mutation.mutate({ submit: false })}>下書き保存</button><button className="primary-button" disabled={mutation.isPending} onClick={() => mutation.mutate({ submit: true })}>根拠を確認して提出</button></div>
      </section></div></>
}
