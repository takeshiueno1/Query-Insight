...
export default function App() {
  return <Routes>
    ...
    <Route element={<ProtectedLayout />}>
      ...
      <Route path="masters" element={<FeaturePage eyebrow="管理設定" title="マスタ管理" description="評価基準版とスキル・資格マスタを履歴管理します" sections={['評価基準版', 'スキルマスタ', '資格マスタ']} />} />
      <Route path="audit" element={<AuditPage />} />
      <Route path="password/change" element={<FeaturePage eyebrow="アカウント" title="パスワード変更" description="現在のパスワード確認後、半角英数字8～128文字（英字・数字を各1文字以上）の新しいパスワードへ変更します" sections={['本人確認', '入力条件検査', '全セッション失効']} />} />
      <Route path="password/reset" element={<FeaturePage eyebrow="アカウント" title="パスワード再設定" description="一回限り・30分有効のトークンで再設定します" sections={['本人確認', 'トークン検証', '再設定完了']} />} />
      <Route path="approvals" element={<Navigate to="/approvals/talent" replace />} />
      <Route path="evaluations/history" element={<FeaturePage eyebrow="評価" title="評価履歴比較" description="期間ごとの自己・上長・確定評価を混同せず比較します" sections={['期間選択', '6軸比較', '根拠スナップショット']} />} />
      <Route path="organization" element={<FeaturePage eyebrow="組織管理" title="組織・所属管理" description="部署階層と所属・上長関係を有効期間付きで管理します" sections={['部署ツリー', '所属履歴', '異動登録']} />} />
      <Route path="accounts" element={<FeaturePage eyebrow="管理設定" title="アカウント・権限管理" description="ロール、データスコープ、有効期間、付与理由を管理します" sections={['アカウント', '権限付与', '棚卸し']} />} />
